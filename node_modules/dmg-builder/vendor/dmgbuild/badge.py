...
def badge_disk_icon(badge_file, output_file):
    ...
    url = CFURLCreateWithFileSystemPath(None, _REMOVABLE_DISK_PATH, kCFURLPOSIXPathStyle, False)
    ...
    for n in range(backdropCount):
        ...
        if width >= 16:
            ...
            perspective.setValue_forKey_(CIVector.vectorWithX_Y_(*topLeft), "inputTopLeft")
            perspective.setValue_forKey_(CIVector.vectorWithX_Y_(*topRight), "inputTopRight")
            perspective.setValue_forKey_(CIVector.vectorWithX_Y_(*bottomRight), "inputBottomRight")
            perspective.setValue_forKey_(CIVector.vectorWithX_Y_(*bottomLeft), "inputBottomLeft")
            out = perspective.valueForKey_(kCIOutputImageKey)

        tfm = NSAffineTransform.transform()
        tfm.translateXBy_yBy_(
            math.floor((position[0] - 0.5 * scale) * width), math.floor((position[1] - 0.5 * scale) * height)
        )
        ...
        cgContext = CGBitmapContextCreate(None, width, height, 8, 0, rgbColorSpace, kCGImageAlphaPremultipliedLast)
        context = CIContext.contextWithCGContext_options_(cgContext, None)

        context.drawImage_inRect_fromRect_(result, ((0, 0), (width, height)), ((0, 0), (width, height)))
