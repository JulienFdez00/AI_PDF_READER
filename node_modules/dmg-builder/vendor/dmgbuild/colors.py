...
class ColorParser(object):
    ...
    def expect(self, s, context=""):
        if len(self._string) - self._pos < len(s) or self._string[self._pos : self._pos + len(s)] != s:
            raise ValueError('bad color "%s" - expected "%s"%s' % (self._string, s, context))
        self._pos += len(s)
    ...
    def parseNumber(self, context=""):
        ...
        raise ValueError('bad color "%s" - expected a number%s' % (self._string, context))
...
_color_re = re.compile("\s*(#|rgb|hsl|hwb|cmyk|gray|grey|%s)" % "|".join(_x11_colors.keys()))
