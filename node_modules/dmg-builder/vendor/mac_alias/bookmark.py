...
class Bookmark(object):
    ...
    def from_bytes(cls, data):
        ...
        while tocoffset != 0:
            ...
            tocsize, tocmagic, tocid, nexttoc, toccount = struct.unpack(b"<IIIII", data[tocbase : tocbase + 20])
    ...
    def _encode_item(cls, item, offset):
        if item is True:
            ...
            secs = item - osx_epoch
            result = struct.pack(b"<II", 8, BMK_DATE | BMK_ST_ZERO) + struct.pack(b">d", float(secs.total_seconds()))
            ...
            if item.base:
                ...
                result = b"".join(
                    [struct.pack(b"<IIII", 8, BMK_URL | BMK_URL_ST_RELATIVE, baseoff, reloff), baseenc, relenc]
                )
            else:
                encoded = item.relative.encode("utf-8")
                result = struct.pack(b"<II", len(encoded), BMK_URL | BMK_URL_ST_ABSOLUTE) + encoded
    ...
    def to_bytes(self):
        ...
        for tid, toc in self.tocs:
            ...
            tocs.append((tid, b"".join([struct.pack(b"<III", k, o, 0) for k, o in entries])))
        ...
        for ndx, toc in enumerate(tocs):
            ...
            result.append(struct.pack(b"<IIIII", len(data) - 8, 0xFFFFFFFE, tid, next_offset, len(data) // 12))
            result.append(data)
        ...
        header = struct.pack(b"<4sIIIQQQQI", b"book", offset + 48, 0x10040000, 48, 0, 0, 0, 0, first_toc_offset)
    ...
    def for_file(cls, path):
        ...
        volprops = Data(
            struct.pack(
                b"<QQQ", 0x81 | kCFURLVolumeSupportsPersistentIDs, 0x13EF | kCFURLVolumeSupportsPersistentIDs, 0
            )
        )
