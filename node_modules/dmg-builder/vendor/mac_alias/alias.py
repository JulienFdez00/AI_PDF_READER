...
class VolumeInfo(object):
    ...
    def __repr__(self):
        args = ["name", "creation_date", "fs_type", "disk_type", "attribute_flags", "fs_id"]
        values = []
        for a in args:
            v = getattr(self, a)
...
class TargetInfo(object):
    ...
    def __repr__(self):
        args = ["kind", "filename", "folder_cnid", "cnid", "creation_date", "creator_code", "type_code"]
        values = []
        for a in args:
            v = getattr(self, a)
            values.append(repr(v))
        ...
        kwargs = ["folder_name", "cnid_path", "carbon_path", "posix_path", "user_home_prefix_len"]
        for a in kwargs:
            v = getattr(self, a)
            values.append("%s=%r" % (a, v))
...
class Alias(object):
    ...
    def _from_fd(cls, b):
        ...
        if version == 2:
            ...
            kind, voldate_hr, fstype, disktype, folder_cnid, cnid, crdate_hr, volattrs, reserved = struct.unpack(
                b">hQ4shIIQI14s", b.read(46)
            )
        ...
        alias.volume = VolumeInfo(volname.decode().replace("/", ":"), voldate, fstype, disktype, volattrs, volfsid)
        alias.target = TargetInfo(
            kind, filename.decode().replace("/", ":"), folder_cnid, cnid, crdate, creator_code, type_code
        )
    ...
    def for_file(cls, path):
        ...
        a.target = TargetInfo(kind, filename, folder_cnid, cnid, creation_date, creator_code, type_code)
    ...
    def _to_fd(self, b):
        ...
        if self.target.cnid_path:
            cnid_path = struct.pack(">%uI" % len(self.target.cnid_path), *self.target.cnid_path)
        ...
        if self.volume.network_mount_info:
            b.write(struct.pack(b">hh", TAG_NETWORK_MOUNT_INFO, len(self.volume.network_mount_info)))
            b.write(self.volume.network_mount_info)
            if len(self.volume.network_mount_info) & 1:
                b.write(b"\0")

        if self.volume.dialup_info:
            b.write(struct.pack(b">hh", TAG_DIALUP_INFO, len(self.volume.network_mount_info)))
            b.write(self.volume.network_mount_info)
            if len(self.volume.network_mount_info) & 1:
                b.write(b"\0")
        ...
        if self.target.user_home_prefix_len is not None:
            b.write(struct.pack(b">hhh", TAG_USER_HOME_LENGTH_PREFIX, 2, self.target.user_home_prefix_len))
