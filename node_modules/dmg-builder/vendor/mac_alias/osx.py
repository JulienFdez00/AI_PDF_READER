...
class fsobj_id_t(Structure):
    _fields_ = [("fid_objno", c_uint), ("fid_generation", c_uint)]
...
class attrreference_t(Structure):
    _fields_ = [("attr_dataoffset", c_int), ("attr_length", c_uint)]
...
class kauth_ace(Structure):
    _fields_ = [("ace_applicable", guid_t), ("ace_flags", c_uint)]


class kauth_acl(Structure):
    _fields_ = [("acl_entrycount", c_uint), ("acl_flags", c_uint), ("acl_ace", kauth_ace * 128)]


class kauth_filesec(Structure):
    _fields_ = [("fsec_magic", c_uint), ("fsec_owner", guid_t), ("fsec_group", guid_t), ("fsec_acl", kauth_acl)]


class diskextent(Structure):
    _fields_ = [("startblock", c_uint), ("blockcount", c_uint)]
...
class Point(Structure):
    _fields_ = [("x", SInt16), ("y", SInt16)]


class Rect(Structure):
    _fields_ = [("x", SInt16), ("y", SInt16), ("w", SInt16), ("h", SInt16)]
...
class FinderInfo(Union):
    _fields_ = [("fileInfo", FileInfo), ("folderInfo", FolderInfo)]
...
class vol_capabilities_attr_t(Structure):
    _fields_ = [("capabilities", vol_capabilities_set_t), ("valid", vol_capabilities_set_t)]


class vol_attributes_attr_t(Structure):
    _fields_ = [("validattr", attribute_set_t), ("nativeattr", attribute_set_t)]
...
def _attrbuf_size(attrs):
    ...
    for entry in _attr_info:
        if attrs[entry[0]] & entry[1]:
            if entry[2] is None:
                raise ValueError("Unsupported attribute (%u, %x)" % (entry[0], entry[1]))
            size += entry[2]
    return size
...
def _decode_attrlist_result(buf, attrs, options):
    ...
    if attrs[0] & ATTR_CMN_EXTENDED_SECURITY:
        ...
        class kauth_acl(Structure):
            _fields_ = [("acl_entrycount", c_uint), ("acl_flags", c_uint), ("acl_ace", kauth_ace * ec)]
...
def getattrlist(path, attrs, options):
    ...
    alist = attrlist(
        bitmapcount=5, commonattr=attrs[0], volattr=attrs[1], dirattr=attrs[2], fileattr=attrs[3], forkattr=attrs[4]
    )

    bufsize = _attrbuf_size(attrs)
    buf = create_string_buffer(bufsize)

    ret = _getattrlist(path, byref(alist), buf, bufsize, options | FSOPT_REPORT_FULLSIZE)
