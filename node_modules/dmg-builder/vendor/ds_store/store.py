...
class DSStore(object):
    ...
    def __init__(self, store):
        ...
        with self._get_block(self._superblk) as s:
            self._rootnode, self._levels, self._records, self._nodes, self._page_size = s.read(b">IIIII")
    ...
    def open(cls, file_or_name, mode="r+", initial_entries=None):
        ...
        if mode == "w" or mode == "w+":
            ...
            if not initial_entries:
                ...
                with store.get_block(superblk) as s:
                    s.write(b">IIIII", root, len(levels), len(initial_entries), node_count, page_size)
    ...
    def flush(self):
        ...
        if self._dirty:
            ...
            with self._get_block(self._superblk) as s:
                s.write(b">IIIII", self._rootnode, self._levels, self._records, self._nodes, self._page_size)
        self._store.flush()
    ...
    def _dump_super(self):
        print(
            "root: %u\nlevels: %u\nrecords: %u\nnodes: %u\npage-size: %u"
            % (self._rootnode, self._levels, self._records, self._nodes, self._page_size)
        )
    ...
    def _split(self, node, entry, right_ptr=0):
        ...
        with self._get_block(node) as block, self._get_block(new_right) as right_block:
            ...
            pivot = self._split2([block, right_block], entries, pointers, before, bool(next_node))[0]
    ...
    def _rebalance(self, path, node):
        ...
        with self._get_block(node) as block:
            ...
            if left_node and right_node:
                with self._get_block(left_node) as left, self._get_block(right_node) as right:
                    ...
                    # of three, go for it
                    pivots = self._split2(blocks, entries, pointers, before, bool(next_node))
                    if pivots is None:
                        ptrs = [left_node, node, right_node]
                        pivots = self._split3(blocks, entries, pointers, before, bool(next_node))
                    else:
                        if pivots:
                            ptrs = [left_node, node]
                        else:
            ...
            with self._get_block(node) as block:
                ...
                if next_node:
                    for n in range(count):
                        ...
                        if filename_lc < e_lc or (filename_lc == e_lc and code < e.code):
                            next_node = ptr
                            break
    ...
    class Partial(object):
        ...
        def __getitem__(self, code):
            ...
            try:
                ...
                raise KeyError("no such key - [%s][%s]" % (self._filename, code))
        ...
        def __setitem__(self, code, value):
            ...
            self._store.insert(DSStoreEntry(self._filename, code, entry_type, entry_value))
